...
using ECommerceSample.Areas.Admin.Models;
...
namespace ECommerceSample.Areas.Admin.Controllers
{
    public class ProductController : Controller
    {
        ...
        public ActionResult List(string m , int? id)
        {
            ...
            if (m!= null)
            {
                ViewBag.Mesaj = String.Format("{0} nolu kaydın silme işlemi {1} ", id, m);
            }
        ...
        [HttpPost]
        public ActionResult AddProduct(ProductViewModel model, IEnumerable<HttpPostedFileBase> photo)
        {
            ...
            //    string[] path = Server.MapPath("~/Upload/" + photoName).Split('-');
            //    foreach (var item in path)
            //    {
            //        photoName = item;
            //    }
            //    photo.SaveAs(photoName);
            //}
            ...
            foreach (var file in photo)
            {
                ...
                {                    
                    photoName = Guid.NewGuid().ToString().Replace("-", "") + ".jpg";
                    string path = Path.Combine(Server.MapPath("~/Upload"), photoName);
                    file.SaveAs(path);
                    model.Product.Photo = photoName;
        ...
        {
            result.Tresult = pr.GetObjById(id);
            string photoName = result.Tresult.ProcessResult.Photo;
            string path = Server.MapPath("~/Upload/" + photoName);
